...
git clone --depth=1 https://github.com/kernelbuild-KutuMoba/AnyKernel3 AnyKernel
echo "Done"
...
function sendinfo() {
    curl -s -X POST "https://api.telegram.org/bot$token/sendMessage" \
        ...
        -d "parse_mode=html" \
        -d text="⚒️<b>• ${KBUILD_BUILD_HOST} Kernel •</b>%0A %0A⚙️Build started%0A %0A📢 For <b>${devices}</b> %0A %0A📝 branch <code>$(git rev-parse --abbrev-ref HEAD)</code>(master)%0A %0A🛠️ Using compiler: <code>${KBUILD_COMPILER_STRING}</code>%0A %0A⏳ Started on <code>$(date)</code>%0A %0A🔑 <b>Build Status:</b>#Stable%0A %0A %0A #ANDROID %0A #KERNEL"
        }
